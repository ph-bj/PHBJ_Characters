...
const fileContent = `// Auto-generated specific relationship specs for pairs with co-occurrence >= 8
import { Relationship } from './types';

export const cooccurrenceSpecificRelationships: Relationship[] = [
${specs.map(s => `  { source: '${s.source}', target: '${s.target}', type: ${JSON.stringify(s.type)}, typeZh: ${JSON.stringify(s.typeZh)} },`).join('\n')}
];
`;

fs.writeFileSync('src/cooccurrenceRelationships.ts', fileContent);
console.log('Successfully written src/cooccurrenceRelationships.ts!');
