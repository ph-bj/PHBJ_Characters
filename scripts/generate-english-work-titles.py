...
def write_ts(titles: list[str], zh_en_map: dict[str, str]) -> None:
    all_titles = sorted(set(titles) | set(zh_en_map.values()), key=len, reverse=True)
    lines = [
    ...
    for key, english in zh_en_map.items():
...
def main() -> None:
    ...
    write_ts(titles, zh_en_map)
    print(
        f"Wrote {len(titles)} titles and {len(zh_en_map)} zh→en mappings to {OUTPUT.relative_to(ROOT)}"
    )
