...
function markerOffsetRetention(
  zoomK: number,
  baseZoomK: number,
  maxZoomK: number,
): number {
...
function layoutMapMarkers(
  ...
  for (const group of groups) {
    ...
    const centroidX =
      group.reduce((sum, node) => sum + node.x, 0) / group.length;
    const centroidY =
      group.reduce((sum, node) => sum + node.y, 0) / group.length;
    const sorted = [...group].sort(
      (a, b) =>
        b.location.count - a.location.count ||
        a.location.origin.localeCompare(b.location.origin),
    );
...
function LocationTooltipSection({
  ...
  return (
    <div
      className={showDivider ? "border-t border-[#5d5048]/60 pt-2 mt-2" : ""}
    >
      <div className="flex items-center gap-2 mb-1">
        <span
          ...
          style={{
            backgroundColor:
              locationColors[location.type as LocationType] || "#8b4513",
          }}
        />
        <p className="font-bold text-sm leading-tight">
          {lang === "zh"
            ? location.originZh || location.origin
            : location.origin}
        </p>
      </div>
      ...
      {lang === "en" &&
        location.originZh &&
        location.originZh !== location.origin && (
          <p className="text-[10px] text-amber-200/70 mb-1 font-hans">
            {location.originZh}
          </p>
        )}
      ...
      <p className="text-[10px] text-amber-200/80 mb-1.5">
        {lang === "zh"
          ? locationTypeLabels[location.type as LocationType]?.zh ||
            location.type
          : locationTypeLabels[location.type as LocationType]?.en ||
            location.type}
        {location.count > 0 && (
          <span className="ml-2">
          ...
          {location.firstSnippet
            ...
            .map((part, i) =>
              (location.searchTokens ?? []).includes(part) ? (
                <mark
                  key={i}
                  className="bg-amber-300/80 text-[#2c2420] px-0.5 rounded-sm"
                >
                  {part}
                </mark>
              ) : (
...
export function LocationMapPanel({
  ...
  useEffect(() => {
    if (!svgRef.current || !containerRef.current || mapData.length === 0)
      return;
    ...
    const capitalStar = capitalStarLayer
      .append("g")
      .attr("class", "capital-star")
      .attr(
        "transform",
        `translate(${beijingProjected[0]},${beijingProjected[1]})`,
      );
    ...
    const updateMarkerTransforms = (scale: number) => {
      ...
      capitalStarLayer
        .select("g.capital-star")
        .attr(
          "transform",
          `translate(${beijingProjected[0]},${beijingProjected[1]}) scale(${inverse})`,
        );

      leaderLayer
        .selectAll<SVGLineElement, MapMarker>("line.marker-leader")
        .each(function (marker) {
          const { x, y } = getMarkerDisplayPosition(
            marker,
            scale,
            initialTransform.k,
            maxZoomK,
      ...
      markerLayer
        .selectAll<SVGGElement, MapMarker>("g.marker")
    ...
    const markerGroups = markerLayer
      .selectAll<SVGGElement, MapMarker>("g.marker")
      .data(markers, (d) => d.id)
      .enter()
      .append("g")
  ...
  useEffect(() => {
    ...
    if (selectedLocation) {
      const selected = svg
        .selectAll(".marker")
        .filter((d: any) => d.id === selectedLocation.id);

      selected.classed("is-selected", true);
    ...
    <div>
      ...
      >
        ...
        <div className="absolute bottom-2 left-2 text-[9px] text-[#5d5048] opacity-70 pointer-events-none">
          {lang === "zh"
            ? "点击圆点查看详情，滚动缩放"
            : "Click dots for details, scroll to zoom"}
        </div>
      </div>

      {selectedLocation && (
        <div className="mt-4 p-4 rounded bg-[#2c2420] text-[#f4ecd8] border border-[#5d5048] relative shadow-md">
          <button
            onClick={() => setSelectedLocation(null)}
            className="absolute top-2 right-2 w-6 h-6 flex items-center justify-center rounded-full hover:bg-[#5d5048]/50 text-[#f4ecd8] transition-colors"
