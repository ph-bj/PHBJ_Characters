    ...
    <dialog ref={dialogRef} aria-labelledby="paragraph-cinema-title" aria-describedby="paragraph-cinema-description"
      ...
      <div className="relative overflow-hidden bg-[#101b22]">
        ...
        <div className="pointer-events-none absolute inset-x-0 bottom-0 flex h-8 items-center justify-between bg-[#0a1015] px-5 text-[9px] uppercase tracking-[0.2em] text-[#c9af85] sm:h-9">
          <span>{isPrologue ? `${String(shotIndex + 1).padStart(2, '0')} / 04 · ${shot.title[lang]}` : `${sceneLabels.mood[plan.mood][lang]} / ${sceneLabels.weather[plan.weather][lang]}`}</span><span className="hidden sm:inline">{zh ? '光影意境' : 'A literary miniature'}</span>
        </div>
      ...
      <div className="px-5 py-4 sm:px-7">
        ...
        {isPrologue && <div className="mt-4">
          <div aria-label={zh ? '选择分镜' : 'Choose a scene'} className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        ...
        <p id="paragraph-cinema-description" className="mt-4 text-xs leading-relaxed text-[#a7b1b3]">{isPrologue ? zh ? '由京城的繁华写到用情守礼，再收于作者笔下。画中人物为本段所写的无名君子与优伶。' : 'From the capital’s splendour to feeling with dignity, and finally the author’s page. The figures represent the unnamed gentlemen and performers in this passage.' : zh ? '依本段文字中的场景与情绪营造的风格化意境。' : 'A stylized interpretation of the setting and mood suggested by this paragraph.'}</p>
