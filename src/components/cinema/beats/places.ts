...
const PAINTERS: Record<Place, (ctx: Ctx, t: number, rand: () => number) => void> = {
  ...
  hall: ctx => {
    ...
    ctx.fillStyle = INK; ctx.font = '32px "KaiTi", "STKaiti", serif'; ctx.textAlign = 'center'; ctx.fillText('世德堂', 512, 102);
  ...
  arch: (ctx, t, rand) => {
    ...
    ctx.fillStyle = INK; ctx.font = '22px "KaiTi", "STKaiti", serif'; ctx.textAlign = 'center'; ctx.fillText('贞烈流芳', 512, 218);
  ...
  'still-wine': ctx => {
    ...
    ctx.fillStyle = PAPER; ctx.fillRect(300, 330, 60, 90); ctx.fillStyle = INK; ctx.font = '48px "KaiTi", "STKaiti", serif'; ctx.textAlign = 'center'; ctx.fillText('酒', 330, 392);
