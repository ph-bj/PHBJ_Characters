  ...
  return (
    <div className="parchment p-3 rounded-sm border-double border-4 border-[var(--paper-border)] flex flex-col items-center my-6 opacity-90 transition-opacity hover:opacity-100 max-w-md mx-auto">
      <svg viewBox="0 0 400 240" className="w-full" style={{ maxHeight: '320px' }}>
        ...
        <g filter="url(#snow-cat-wobble)" stroke="var(--art-ink)" fill="none">
          {/* Eaves / Platform */}
          ...
          {/* Go Board */}
          <path d="M 220 160 L 300 160 L 290 180 L 210 180 Z" fill="var(--art-paper-alt)" strokeWidth="1.5" />
          ...
          {/* Snow scattered */}
          <circle cx="230" cy="165" r="1.5" fill="var(--art-ink)" />
          <circle cx="260" cy="170" r="1.5" fill="var(--art-ink)" />
      ...
      <text x="376" y="220" textAnchor="middle" fill="var(--art-seal)" fontSize="10" fontFamily="serif" fontWeight="bold">猫</text>

      {/* ===== CALLIGRAPHY TITLE (题字) ===== */}
      <text x="382" y="30" textAnchor="middle" fill="var(--art-ink)" fontSize="14" fontFamily="serif" writingMode="vertical-rl" opacity="0.7">雪猫</text>
    </svg>
    </div>
  );
}
