  ...
  return (
    <div className="parchment p-3 rounded-sm border-double border-4 border-[var(--paper-border)] flex flex-col items-center my-6 opacity-90 transition-opacity hover:opacity-100 max-w-md mx-auto">
      <svg viewBox="0 0 400 240" className="w-full" style={{ maxHeight: '320px' }}>
        ...
        <g filter="url(#snow-sound-wobble)" stroke="var(--art-ink)" fill="none">
          {/* Window Frame */}
          ...
          {/* Leaves */}
          <path d="M 220 100 Q 240 105 250 120 Q 230 115 220 100 Z" fill="var(--art-ink)" opacity="0.7" />
          <path d="M 220 100 Q 235 90 240 70 Q 225 80 220 100 Z" fill="var(--art-ink)" opacity="0.7" />
          <path d="M 240 80 Q 260 85 270 100 Q 250 95 240 80 Z" fill="var(--art-ink)" opacity="0.5" />
          ...
          <circle cx="100" cy="70" r="1" fill="var(--art-ink)" opacity="0.6" />
          <circle cx="120" cy="90" r="1" fill="var(--art-ink)" opacity="0.6" />
          <circle cx="180" cy="110" r="1" fill="var(--art-ink)" opacity="0.6" />
          <circle cx="210" cy="60" r="1" fill="var(--art-ink)" opacity="0.6" />
          <circle cx="150" cy="50" r="1" fill="var(--art-ink)" opacity="0.6" />
          <circle cx="80" cy="130" r="1" fill="var(--art-ink)" opacity="0.6" />
        </g>
      
      {/* ===== RED SEAL STAMP (印章) ===== */}
      <rect x="365" y="205" width="22" height="22" fill="none" stroke="var(--art-seal)" strokeWidth="1.5" rx="1" />
      <text x="376" y="220" textAnchor="middle" fill="var(--art-seal)" fontSize="10" fontFamily="serif" fontWeight="bold">声</text>

      {/* ===== CALLIGRAPHY TITLE (题字) ===== */}
      <text x="382" y="30" textAnchor="middle" fill="var(--art-ink)" fontSize="14" fontFamily="serif" writingMode="vertical-rl" opacity="0.7">雪声</text>
    </svg>
    </div>
  );
}
